...
tokit是一个用于方便网游研发人员配置数据的工具，唯一需要的东西是一份excel文件，目前仅支持生成c++。

## 使用过程

* 1. 策划在excel中编辑完数据后
* 2. 程序人员在同一个excel中编辑好字段定义
* 3. 使用tokit从excel中生成c++代码，并导出xml数据文件和xsd文件。

详细方法可见目录下的《Tokit使用手册.pdf》。

## 下载之后

下载tokit整个项目之后，运行example目录下的《一键生成.bat》脚本，该脚本将根据example\excel中的各个excel文件，生成对应的xml、xsd、c++文件。

参考example\c++\c++_example.sln项目，将tokit接入你自己的c++项目

## 示例


为了让打算下载tokit的人更快的了解tokit，我把tokit生成的文件全部贴出来。
...
项目跟tokit对接后，载入数据的工作很简单，添加一行语句即可： dotaherocfgmgr::instance().load();